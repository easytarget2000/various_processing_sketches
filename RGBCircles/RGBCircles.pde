...
void draw() {
  ...
  if (conductor.isBeatDue(2) && random(1f) > 0.5f) {
    background(0);
  }

  if (conductor.isBeatDue(2) && random(1f) > 0.2f) {
    rotateZ(frameCount * 0.0025f);
  } else {
    rotateZ(0f);
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (float x = diameter / 2f; x < (width * 1.2f); x += diameter * 1.1f) {
  ...
  if (conductor.isBeatDue(4) && random(1f) > 0.7f) {
    initShapes();
  } else {
    updateAngles();
  }

  if (conductor.isBeatDue(1) && random(1f) > 0.9f) {
    initPaint();
  }
}
