...
private static final int DEFAULT_NUM_OF_PARTICLES = 64;
...
private void updateAndDrawParticles() {
  ...
  for (final Particle currentParticle : particles) {
    ...
    final PVector forceOnParticle = PVector.mult(vectorToAttractor, attractorSign * 4f);
    currentParticle.update(particles, forceOnParticle);
    currentParticle.draw_(1f);
  }
}
